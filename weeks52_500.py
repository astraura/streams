...
st.write("Index Used: Nifty 500 Multicap 50:25:25 ")
...
def getNifty(niftyapiurl, csvfile, csv2):
    ...
    with requests.session() as req:
        ...
        api0= req.get(niftyapiurl, headers = headers,timeout=12)
...
def getChartink():
    ...
    for condition in conditions:
        ...
        with st.spinner('Wait. Gathering {}..'.format(msg) ):
            ...
            cdatas.append(data)
    cdatas[0].to_csv(files[0]) 
    cdatas[1].to_csv(files[1]) 
...
if curdate  >last_update:
    #getFirst()
    with st.spinner('Wait. Data needs updation. Gathering 52w weeks data...'):
        gatherData()
    ...
    #,,
    #cdatas=[]
    #cdatas.append(pd.read_csv("Nifty500 52wkNewHigh.csv", index_col=0))
    #cdatas.append(pd.read_csv("Nifty500 52wkFiltered.csv", index_col=0))
    df0= df500 # pd.read_csv("Nifty_500multi.csv", index_col=[0])
    indret = ret500.loc[0,'Return']
    nifty_list = pd.read_csv('NSE_symbols.csv')
    #dfc0 = cdatas[0]
...
with tab1:
    ...
    st.write(dft0)
with tab2:
    st.write('Stocks showing new 52 week highs and Relative Strength above 70/100')
    ...
    #st.write("Filtered stocks above RS rating 70")
    #dfcx0f = dfcx0[dfcx0['RS_Rating']>70].copy()
    #dfcx0f=  dfcx0f[ dfcx0f['1 Year Return%']>12]
    
    #write_formatted(dfcx0f, 3)
...
with tab4:
    ...
    st.write("Chart showing  sector wise contribution to Index Investment appreciation. Not average returns! ")
    st.plotly_chart(fig, use_container_width=True)


    with st.expander("See Stocks in each Sector: "):

        for sector, values in dfgr:
            st.write (sector)
            st.write (values[['Stock Name', 'Symbol']])
...
with tab6:
    ...
    url3 = "https://share.streamlit.io/astraura/streams/main/weeks52_250.py"
